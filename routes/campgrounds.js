...
router.get("/:slug", async function (req, res) {
  try {
    ...
    let foundCampground = await Campground.findOne({
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      var asset = await cloudinary.uploader.upload(req.file.path, {
        invalidate: true,
      });
...
async function deletePhotoOnCloud(photoId) {
  try {
    var result = await cloudinary.uploader.destroy(photoId, {
      invalidate: true,
    });
